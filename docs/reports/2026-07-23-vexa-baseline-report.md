...
validate the tool on production code and scope the vexa adoption effort.
...
diagnostics anywhere. Crash gate: clean on both projects (542 vexa source
files, all parsed).
...
- **`sqlite_store.rs` size** - expected ~4.7k lines. Actual: `wc -l` = 4746
  ...
  (max 150). Confirmed, this is the single worst offender in vexa (34
...
- **Inline SQL** - expected. Confirmed: 32 `no-inline-sql` hits across
  ...
  other files carry this rule in vexa.
- **dj-rocket `lib.rs` commands** - expected. Confirmed: every
  ...
  declarations bring the file to 29 diagnostics, by far dj-rocket's worst
  file.
...
dj-rocket is much smaller (50 errors, no tips) and can likely go straight
